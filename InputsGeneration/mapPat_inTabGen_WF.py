...
def main():
     ...
     parser.add_argument('-db', '--database', choices=['GISAID', 'NCBI'], help='Name of the database from which metadata are dowloaded. Allowed values: GISAID, NCBI.')
     ...
     elif inputs.database=='NCBI' and inputs.pathogen=='mPox':
         command('mkdir %s_mapPatInterOut'%(inputs.output_file))
         command('mkdir %s_mapPatOut'%(inputs.output_file))
         
         command('perl ./Scripts/addToTableNCBI.pl --metadata %s --seq %s --outfile MpoxTable.HaploCoV --nproc 18 --ref ./Config/mPox_reference.fasta'%(inputs.input_file, inputs.seq))
